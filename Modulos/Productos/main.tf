...
resource "aws_iam_policy" "products_lambda" {
  ...
  description = "Acceso minimo de productos a DynamoDB y CloudWatch Logs"
...
resource "aws_api_gateway_rest_api" "products" {
  name        = "productos-api"
  description = "API del modulo de gestion de productos"
...
resource "aws_api_gateway_deployment" "products" {
  ...
  triggers = {
    redeployment = sha1(jsonencode({
      resources    = [for resource in [aws_api_gateway_resource.productos, aws_api_gateway_resource.product, aws_api_gateway_resource.inventario, aws_api_gateway_resource.tiendas, aws_api_gateway_resource.store, aws_api_gateway_resource.store_products] : resource.id]
...
resource "aws_iam_policy" "products_api_administrador" {
  ...
  description = "Permite administracion completa de productos"
